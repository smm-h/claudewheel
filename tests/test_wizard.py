"""Tests for create_profile(self.ws, ) and run_profile_wizard() in claudewheel.wizard."""
...
def _paste_terminal(*pastes: str) -> FakeTerminal:
    """A FakeTerminal that feeds each pasted token as keystrokes + ENTER.

    Drives the wizard's masked-input primitive (Terminal.read_masked_line):
    every character of *pastes* is delivered as a key, each paste terminated
    by ENTER. When the keys run out the FakeTerminal safety net yields ESC,
    which the masked primitive treats as an abort (KeyboardInterrupt).
    """
    keys: list[str] = []
    for p in pastes:
        keys.extend(list(p))
        keys.append("ENTER")
    return FakeTerminal(keys)
...
class CreateProfileTestBase(unittest.TestCase):
    """Base class that sets up an isolated home dir for create_profile(self.ws, )."""
    ...
    def setUp(self) -> None:
        ...
        # Suppress create_profile(self.ws, ) print output
        ...
        self._home_patch = mock.patch.object(
            Path, "home", return_value=self.fake_home, autospec=True
        )
        ...
        # create_profile(self.ws, ) takes an explicit workspace; build one rooted at the
        # sandbox launcher dir.
        ...
        self.ws = Workspace.open(
            self.launcher_dir, claude_dir=self.fake_home / ".claude"
        )
        self._scripts_dir = self.ws.scripts_dir
        self._shared_settings_file = self.ws.shared_settings_file
    ...
    def _read_settings(self, name: str = "test") -> dict[str, Any]:
        data: dict[str, Any] = json.loads(
            (self._profile_dir(name) / "settings.json").read_text()
        )
        return data

    def _expected_hooks(self) -> dict[str, Any]:
        """Return the canonical hooks template using the test scripts dir."""
        hooks: dict[str, Any] = build_canonical_shared_settings(self._scripts_dir)[
            "hooks"
        ]
        return hooks
...
class SettingsFromDefaultsTests(CreateProfileTestBase):
    ...
    def test_no_shared_settings_file_produces_hardcoded_defaults(self) -> None:
        ...
        self.assertEqual(
            settings["claudewheel"], {"disallowedTools": DISALLOWED_TOOLS[:]}
        )
    ...
    def test_malformed_shared_settings_uses_canonical_defaults(self) -> None:
        ...
        self.assertFalse(settings["awaySummaryEnabled"])
        self.assertEqual(
            settings["cleanupPeriodDays"], profile_defaults["cleanupPeriodDays"]
        )
...
class CloneFromDefaultTests(CreateProfileTestBase):
    ...
    def _write_default_settings(self) -> dict[str, Any]:
...
class CheckboxOverridesTests(CreateProfileTestBase):
    ...
    def test_all_overrides_combined(self) -> None:
        result = _make_result(disable_recap=True, cleanup_10y=True, disable_memory=True)
        create_profile(self.ws, result)
        settings = self._read_settings()
        self.assertFalse(settings["awaySummaryEnabled"])
        self.assertEqual(settings["cleanupPeriodDays"], 3650)
    ...
    def test_no_overrides_preserves_profile_defaults(self) -> None:
        ...
        profile_defaults = canonical["profileDefaults"]
        self.assertEqual(
            settings["awaySummaryEnabled"], profile_defaults["awaySummaryEnabled"]
        )
        self.assertEqual(
            settings["cleanupPeriodDays"], profile_defaults["cleanupPeriodDays"]
        )
        self.assertEqual(
            settings["autoMemoryEnabled"], profile_defaults["autoMemoryEnabled"]
        )
...
class SymlinkCreationTests(CreateProfileTestBase):
    ...
    def test_no_symlinks_when_checkbox_off(self) -> None:
        ...
        for dirname in SharedStore.SHARED_SUBDIRS:
            self.assertFalse(
                (profile_dir / dirname).exists(), f"{dirname} should not exist"
            )
            self.assertFalse(
                (profile_dir / dirname).is_symlink(),
...
class WizardTUITestBase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        # Workspace rooted so ws.profiles_dir == fake_profiles_dir; _validate_name
        # checks against it.
        from claudewheel.workspace import Workspace
    ...
    def _run_wizard(
        ...
        return run_profile_wizard(self.ws, existing_profiles, THEME, self.fake_term)
...
class EnterFromNameSubmitsTests(WizardTUITestBase):
    ...
    def test_config_dir_matches_name(self) -> None:
        ...
        result = self._run_wizard(keys)
        self.assertEqual(result.config_dir, str(self.ws.profiles.path_for("myname")))
...
class RelocatedWorkspaceConfigDirTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.ws = Workspace.open(
            self.relocated_root, claude_dir=self.relocated_root / ".claude"
        )
    ...
    def test_config_dir_is_relocated_path(self) -> None:
        ...
        self.assertEqual(str(Path(result.config_dir).expanduser()), result.config_dir)
...
class AuthFlowTestBase(unittest.TestCase):
    """Shared setup for run_auth_flow(self.ws, self.locator, ) tests.
    ...
    def setUp(self) -> None:
        ...
        self._browsers_mock = self._browsers_patch.start()
        ...
        (self.fake_home / ".claudewheel").mkdir(parents=True, exist_ok=True)
        self.ws = Workspace.open(
            self.fake_home / ".claudewheel", claude_dir=self.fake_home / ".claude"
        )
        self.locator = BinaryLocator.default()
        self.state_file = self.ws.state_file
...
class AuthFlowTests(AuthFlowTestBase):
    """Tests for run_auth_flow(self.ws, self.locator, ) post-wizard auth setup.
    ...
    def test_skip_choice_returns_skip(self) -> None:
        ...
        with mock.patch(
            "claudewheel.wizard.run_selection", autospec=True, return_value="skip"
        ):
    ...
    def test_form_cancel_returns_cancel(self) -> None:
        ...
        with mock.patch(
            "claudewheel.wizard.run_selection", autospec=True, return_value=None
        ):
            result = run_auth_flow(
                self.ws,
                self.locator,
    ...
    def test_selection_options_and_flags(self) -> None:
        ...
        self.assertEqual(
            [key for key, _label in args[1]], ["session", "token", "paste", "skip"]
        )
    ...
    def test_session_login_binary_not_found(self) -> None:
        ...
        with (
            ...
            mock.patch(
                "claudewheel.wizard.shutil.which", return_value=None, autospec=True
            ),
        ):
            result = run_auth_flow(
                self.ws,
    ...
    def test_session_login_success(self) -> None:
        ...
        with (
            ...
            mock.patch(
                "claudewheel.wizard.subprocess.run", side_effect=fake_run, autospec=True
            ),
        ):
            result = run_auth_flow(
                self.ws, self.locator, config_dir_str, "authtest", THEME, self.term
            )
    ...
    def test_session_login_no_credentials(self) -> None:
        ...
        with (
            ...
            mock.patch(
                "claudewheel.wizard.subprocess.run",
                return_value=subprocess.CompletedProcess([], 0),
                autospec=True,
            ),
        ):
            result = run_auth_flow(
                self.ws, self.locator, config_dir_str, "nocred", THEME, self.term
            )
        self.assertEqual(result, "failed")
    ...
    def test_long_lived_token_success(self) -> None:
        ...
        No paste prompt appears on the happy path: no keys are ever read.
        """
        from claudewheel import auth
        from claudewheel.wizard import run_auth_flow

        fake_binary = self._make_fake_binary()
        self.term = _paste_terminal()
        ...
        with (
            ...
            mock.patch(
                "claudewheel.wizard.run_under_pty",
                autospec=True,
    ...
    def test_long_lived_token_subprocess_error(self) -> None:
        ...
        with (
            ...
            mock.patch(
                "claudewheel.wizard.run_under_pty", autospec=True, return_value=(1, b"")
            ),
        ):
            result = run_auth_flow(
                self.ws,
    ...
    def test_long_lived_token_pty_error(self) -> None:
        ...
        self.assertEqual(result, "failed")
        self.assertIn("Error running claude setup-token", self._stdout_buf.getvalue())
    ...
    def test_long_lived_token_empty_recovery_paste(self) -> None:
        ...
        fake_binary = self._make_fake_binary()
        self.term = _paste_terminal("")  # empty recovery paste (just ENTER)
    ...
    def test_long_lived_token_recovery_paste_probe_gated(self) -> None:
        ...
        fake_binary = self._make_fake_binary()
        self.term = _paste_terminal("sk-ant-recovered")
    ...
    def test_long_lived_token_keyboard_interrupt_on_recovery_paste(self) -> None:
        ...
        fake_binary = self._make_fake_binary()
        self.term = FakeTerminal(["CTRL_C"])  # Ctrl-C during the recovery paste
    ...
    def test_find_claude_binary_falls_back_to_which(self) -> None:
        ...
        with mock.patch(
            "claudewheel.wizard.shutil.which",
            return_value="/usr/bin/claude",
            autospec=True,
        ):
            result = _find_claude_binary(self.locator)
...
class BrowserSelectionTests(AuthFlowTestBase):
    ...
    def test_browser_form_shown_after_session_choice(self) -> None:
        ...
        self.assertEqual(
            args[1], [("/usr/bin/firefox", "Firefox"), ("copy", "Copy URL instead")]
        )
        self.assertNotIn("use_alt_screen", kwargs)
    ...
    def test_no_browser_form_on_skip(self) -> None:
        ...
        with mock.patch(
            "claudewheel.wizard.run_selection", autospec=True, side_effect=["skip"]
        ) as mock_sel:
            result = run_auth_flow(
                self.ws,
                self.locator,
    ...
    def test_no_browser_form_on_cancel(self) -> None:
        ...
        with mock.patch(
            "claudewheel.wizard.run_selection", autospec=True, side_effect=[None]
        ) as mock_sel:
            result = run_auth_flow(
                self.ws,
                self.locator,
    ...
    def test_esc_on_browser_form_cancels(self) -> None:
        ...
        with (
            ...
            mock.patch("claudewheel.wizard.subprocess.run", autospec=True) as mock_run,
        ):
            result = run_auth_flow(
                self.ws,
                self.locator,
    ...
    def test_zero_browsers_shows_copy_only(self) -> None:
        ...
        self._browsers_mock.return_value = []
        with mock.patch(
            "claudewheel.wizard.run_selection",
            autospec=True,
            side_effect=["session", None],
...
class AuthBrowserPersistenceTests(AuthFlowTestBase):
    ...
    def _successful_session_run(self, browser_choice: str) -> str:
        ...
        with (
            ...
            return run_auth_flow(
                self.ws, self.locator, str(config_dir), "authtest", THEME, self.term
            )
    ...
    def test_browser_persisted_after_successful_session_auth(self) -> None:
        ...
        self.assertEqual(result, "authenticated")
        self.assertEqual(self._read_state().get("auth_browser"), "/usr/bin/firefox")
    ...
    def test_remembered_browser_passed_as_initial_key(self) -> None:
        ...
        self.state_file.write_text(json.dumps({"auth_browser": "/usr/bin/firefox"}))
    ...
    def test_stale_browser_still_passed_as_initial_key(self) -> None:
        ...
        args, kwargs = mock_sel.call_args_list[1]
        self.assertEqual(kwargs.get("initial_key"), "/usr/bin/uninstalled-browser")
        # The stale path is not among the offered options
        self.assertNotIn(
            "/usr/bin/uninstalled-browser", [key for key, _label in args[1]]
        )
...
class CookedWindowTests(AuthFlowTestBase):
    ...
    def _track_cooked(self, events: list[str]) -> None:
        ...
        cm.__exit__.side_effect = _record_exit
    ...
    def test_token_pty_capture_runs_inside_cooked_window(self) -> None:
        ...
        def fake_pty(argv: list[str], env: dict[str, str]) -> tuple[int, bytes]:
            events.append("pty")
        ...
        with (
            ...
            mock.patch(
                "claudewheel.wizard.run_under_pty", autospec=True, side_effect=fake_pty
            ),
            mock.patch(
                "claudewheel.auth.validate_token",
                autospec=True,
    ...
    def test_recovery_paste_read_via_masked_primitive(self) -> None:
        """When extraction fails, the recovery paste is read (masked) and saved.

        The masked-input primitive owns its own raw-mode transitions, so the
        recovery paste no longer relies on the surrounding cooked() window; the
        important guarantee is that the pasted token is read and the flow
        authenticates. The PTY-in-cooked invariant is covered separately by
        test_token_pty_capture_runs_inside_cooked_window.
        """
        from claudewheel import auth
        from claudewheel.wizard import run_auth_flow
...
class TokenValidationRedGreenTests(AuthFlowTestBase):
    ...
    def test_invalid_token_never_saved_and_not_authenticated(self) -> None:
        ...
        fake_binary = self._make_fake_binary()
        self.term = _paste_terminal("sk-ant-oat01-TRUNCATED")
...
class TokenRecoveryPasteTests(AuthFlowTestBase):
    ...
    def _run_recovery_flow(
        ...
        self.term = _paste_terminal(pasted)
    ...
    def test_surrounding_whitespace_removed(self) -> None:
        ...
        self.assertEqual(result, "authenticated")
        mock_add.assert_called_once_with(
            mock.ANY, "test", "sk-ant-token-1", expiry=TTL
        )
...
class TokenValidationOutcomeTests(AuthFlowTestBase):
    ...
    def _run_scraped_flow(
        ...
        reprompt: str = "",
        captured: bytes = CAPTURED_OUTPUT,
    ) -> tuple[
        Any,
        mock.MagicMock,
        mock.MagicMock,
        ...
        reprompt: the paste fed to the INVALID re-prompt (via the FakeTerminal).
        """
        from claudewheel.wizard import run_auth_flow
        ...
        return result, mock_add, mock_probe, self.term, mock_sel

    def test_valid_token_saved_no_paste_prompt(self) -> None:
        """VALID: the scraped token is saved; input() is never called."""
        from claudewheel import auth

        result, mock_add, mock_probe, term, _sel = self._run_scraped_flow(
            [auth.VALID]
        )
        self.assertEqual(result, "authenticated")
        mock_add.assert_called_once_with(mock.ANY, "test", CAPTURED_TOKEN, expiry=TTL)
    ...
    def test_invalid_twice_fails_never_saved(self) -> None:
        ...
        result, mock_add, mock_probe, term, _sel = self._run_scraped_flow(
            [auth.INVALID, auth.INVALID], reprompt="sk-ant-still-bad"
        )
        self.assertEqual(result, "failed")
        mock_add.assert_not_called()
        # Exactly one re-prompt fired: the paste keys were consumed.
        self.assertGreater(term._index, 0)
    ...
    def test_invalid_then_empty_reprompt_fails(self) -> None:
        ...
        result, mock_add, mock_probe, _input, _sel = self._run_scraped_flow(
            [auth.INVALID], reprompt=""
        )
        self.assertEqual(result, "failed")
        mock_add.assert_not_called()
...
class OnboardingFlagTests(CreateProfileTestBase):
    """create_profile(self.ws, ) must write .claude.json with hasCompletedOnboarding: true."""

    def _read_claude_json(self, name: str = "test") -> dict[str, Any]:
...
class OnboardingFlagAuthTests(AuthFlowTestBase):
    """run_auth_flow(self.ws, self.locator, ) must write .claude.json with hasCompletedOnboarding
    after successful auth (authenticated or unverified)."""

    def _read_claude_json(self, config_dir: str) -> dict[str, Any]:
    ...
    def test_session_login_success_writes_onboarding_flag(self) -> None:
        ...
        with (
            ...
            result = run_auth_flow(
                self.ws, self.locator, config_dir_str, "authonboard", THEME, self.term
            )
    ...
    def test_failed_auth_does_not_write_flag(self) -> None:
        ...
        with (
            ...
            result = run_auth_flow(
                self.ws, self.locator, config_dir_str, "failauth", THEME, self.term
            )

        self.assertEqual(result, "failed")
        cj = self._read_claude_json(config_dir_str)
        self.assertFalse(
            cj.get("hasCompletedOnboarding", False),
...
class HookMergeGapTests(CreateProfileTestBase):
    ...
    def _assert_all_wirings(self, hooks: dict[str, Any]) -> None:
    ...
    def test_clone_with_only_userpromptsubmit_gets_all_wirings(self) -> None:
        ...
        result = _make_result(name="mergegap", clone_from="onlyups", wire_hooks=True)
        create_profile(self.ws, result)
    ...
    def test_clone_custom_ups_hook_preserved_and_all_wired(self) -> None:
        ...
        result = _make_result(name="custmerge", clone_from="custom", wire_hooks=True)
        create_profile(self.ws, result)

        settings = self._read_settings("custmerge")
        self._assert_all_wirings(settings["hooks"])
        cmds = [h["command"] for h in settings["hooks"]["UserPromptSubmit"][0]["hooks"]]
...
class PasteTokenTests(AuthFlowTestBase):
    ...
    def test_paste_valid_token_saves_and_authenticates(self) -> None:
        ...
        self.term = _paste_terminal("sk-ant-oat01-GOODTOKEN123")
    ...
    def test_paste_invalid_then_valid_repaste_saves(self) -> None:
        ...
        self.term = _paste_terminal("sk-ant-bad", "sk-ant-good")
        ...
        self.assertEqual(result, "authenticated")
        mock_add.assert_called_once_with(mock.ANY, "test", "sk-ant-good", expiry=UNKNOWN)
        self.assertEqual(mock_probe.call_count, 2)
        self.assertIn("rejected by the API (401)", self._stdout_buf.getvalue())
    ...
    def test_paste_invalid_twice_fails(self) -> None:
        ...
        self.term = _paste_terminal("sk-ant-bad1", "sk-ant-bad2")
    ...
    def test_paste_unreachable_save_returns_unverified(self) -> None:
        ...
        self.term = _paste_terminal("sk-ant-token")
        ...
        self.assertEqual(result, "unverified")
        mock_add.assert_called_once_with(mock.ANY, "test", "sk-ant-token", expiry=UNKNOWN)
