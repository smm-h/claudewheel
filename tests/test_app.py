...
class StaleTokenReviewTests(unittest.TestCase):
    """Phase 5.2: the persistent stale-token notice and its review page."""

    def setUp(self) -> None:
        self._tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self._tmp.cleanup)
        self.home = Path(self._tmp.name)
        self.launcher = self.home / ".claudewheel"
        self.profiles_dir = self.launcher / "profiles"
        self.profiles_dir.mkdir(parents=True)
        self.tokens_file = self.launcher / "tokens.json"
        from claudewheel.workspace import Workspace

        self.ws = Workspace.open(self.launcher, claude_dir=self.home / ".claude")
    ...
    def _make_app(self, keys: list[str]) -> Any:
        ...
        app = object.__new__(app_mod.App)
        app.workspace = self.ws
        app._flash = ""
        app._orphan_findings = []
        app._notice = ""
        app.theme = parse_theme(DEFAULT_THEME_DARK)
        app.terminal = FakeTerminal(keys, in_raw=True)
        return app

    def test_notice_present_with_findings(self) -> None:
        self._write_tokens({"ghost": {"token": "t"}})
        app = self._make_app([])
        app._refresh_orphan_findings()
        self.assertEqual([f.name for f in app._orphan_findings], ["ghost"])
        self.assertIn("press T to review", app._notice)
        self.assertIn("1 stale token entry", app._notice)

    def test_notice_absent_without_findings(self) -> None:
        self._make_profile("live")
        self._write_tokens({"live": {"token": "t"}})
        app = self._make_app([])
        app._refresh_orphan_findings()
        self.assertEqual(app._orphan_findings, [])
        self.assertEqual(app._notice, "")

    def test_notice_pluralizes(self) -> None:
        self._write_tokens({"ghost": {"token": "t"}, "wraith": {"token": "u"}})
        app = self._make_app([])
        app._refresh_orphan_findings()
        self.assertIn("2 stale token entries", app._notice)

    def test_review_removes_entry_and_clears_notice(self) -> None:
        self._make_profile("live")
        self._write_tokens({"live": {"token": "t"}, "ghost": {"token": "g"}})
        app = self._make_app(["ENTER"])  # first option (ghost) selected
        app._refresh_orphan_findings()
        self.assertIn("ghost", [f.name for f in app._orphan_findings])

        app._show_stale_token_review()

        tokens = json.loads(self.tokens_file.read_text())
        self.assertNotIn("ghost", tokens)
        self.assertEqual(tokens, {"live": {"token": "t"}})
        self.assertEqual(app._orphan_findings, [])
        self.assertEqual(app._notice, "")
        self.assertIn("Removed", app._flash)

    def test_review_cancel_leaves_store_untouched(self) -> None:
        self._write_tokens({"ghost": {"token": "g"}})
        # Options are [ghost, Cancel]; DOWN moves to Cancel, ENTER selects it.
        app = self._make_app(["DOWN", "ENTER"])
        app._refresh_orphan_findings()

        app._show_stale_token_review()

        tokens = json.loads(self.tokens_file.read_text())
        self.assertIn("ghost", tokens)
        self.assertEqual(len(app._orphan_findings), 1)
        self.assertNotEqual(app._notice, "")
        self.assertEqual(app._flash, "")

    def test_T_key_dispatches_to_review_when_findings(self) -> None:
        self._write_tokens({"ghost": {"token": "g"}})
        app = self._make_app([])
        app._refresh_orphan_findings()
        seg = _make_profile_segment(discovered=["ghost"])
        app.bar = mock.MagicMock()
        app.bar.segments = [seg]
        app.bar.focused = seg
        app._show_provenance = False
        ...
        with mock.patch.object(
            app, "_show_stale_token_review", autospec=True
        ) as review:
            app._handle_key("T")
        review.assert_called_once()

    def test_T_hint_hidden_without_findings(self) -> None:
        self._make_profile("live")
        self._write_tokens({"live": {"token": "t"}})
        app = self._make_app([])
        app._refresh_orphan_findings()
        seg = _make_profile_segment(discovered=["live"])
        app.bar = mock.MagicMock()
        app.bar.segments = [seg]
        app.bar.focused = seg
        app._show_provenance = False
        ...
        self.assertNotIn("T: review", app._compute_hints())
