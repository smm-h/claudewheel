...
Claude Code sessions use `safegit` for all git operations (commit, etc.), but agents fall back to raw `git add`, `git stash`, `git restore`, `git checkout --` out of habit. A `deny` permission rule blocks the command but gives no context -- the agent doesn't understand why it was denied or what to use instead. It retries, tries variations, or asks the user.
...
2. Inspects the command string for forbidden git patterns:
