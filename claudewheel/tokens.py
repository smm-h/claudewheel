...
def compute_expiry(
    ...
    Dict entries delegate to :func:`entry_expiry`; a legacy bare-string entry
    carries no dates of its own, so only the tokens.json file mtime
    (+ TOKEN_TTL_DAYS) can date it.
    """
    if isinstance(entry, dict):
        return entry_expiry(entry, today)
...
def _write_token(
    ...
    *expiry* selects how the entry records its lifetime -- see
    ...
    tokens[name] = build_entry(
        token, expiry=expiry, tier=tier, subscription=subscription
    )
...
class TokenStore:
    ...
    def plan_env_for(self, name: str) -> dict[str, str]:
        ...
        if not isinstance(entry, dict):
            return {}
        return entry_plan_env(entry, source=f"Profile {name!r} in {self.path}")
