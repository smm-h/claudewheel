...
def build_frame(
    ...
    *,
    ...
    A row only partly inside the window is **left out**, and the lines it would
    have occupied are drawn blank so the hint keeps its place at the bottom.
    The alternative -- drawing the lines that fit -- was built alongside this
    one and compared on a 24-row terminal: at the bottom edge it gains a header
    naming one more session, but at the top edge it draws a block's indented
    detail line directly above the *next* session's header, where it reads as
    that session's.  A line whose owner is off screen is worse than a blank
    one, and worst on the checklist, where every row is a process someone is
    deciding whether to stop.

    The focused row is the one exception: it is drawn clipped rather than
    hidden, because a window too short for it would otherwise show nothing at
    all.
    """
    ...
    if not rows:
        ...
        for slice_ in viewport.rows:
            ...
            if slice_.clipped and not highlighted:
