...
def check_shared_symlinks(
    ...
    broken: list[str] = []
    for p in profiles:
        states = ws.profiles.classify_shared_dirs(p.name)
        ...
        if ws.skills_dir.is_dir() and states["skills"] != "intact":
...
def check_auth_shadow(
    ...
    shadowed: list[str] = []
    for p in profiles:
        if detect_auth_shadow(ws, p.name):
